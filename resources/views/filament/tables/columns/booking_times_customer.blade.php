<div class="flex flex-col">
    @foreach ($getRecord()->bookingTimes as $item)
        @if ($item->status == 0)
            <span class="mt-1 px-2 py-1 font-semibold text-sm bg-cyan-500 text-white rounded-full shadow-sm">{{ \Carbon\Carbon::createFromFormat('H:i:s', $item->time)->format('h:i a') }}</span>
